...
Router.get('/create', (req, res)=>{
    ...
    res.status(200).render('../View/Registrar-Ficha-Tecnica');
...
Router.get('/update/:id', (req, res)=>{
    ...
    conexion.query(`SELECT id_ft,cantidad_insumo, costo_insumo,imagen_final_producto, costo_final_producto, deatalle FROM ficha_tecnica WHERE id_ft='${id}'` , function(error, result, fields){
        if(error){
            throw error;
        }else{
    ...
    Router.post('/update/in/',(req, res)=>{ 
        ...
        const imagen_producto_final=req.body.imagen_producto_final;
        const costo_final_producto=parseFloat(req.body.costo_final_producto);
        const detalle=parseInt(req.body.detalle);
    
        
    
        conexion.query(`UPDATE ficha_tecnica SET fk_insumo=${insumo}, cantidad_insumo='${cantidad_insumo}', costo_insumo=${costo_insumo},imagen_final
        _producto='${imagen_producto_final}',costo_final_producto='${costo_final_producto}',detallle =${detalle}WHERE id_ft='${id}'`,function(error, result, fields){
            if(error){
                console.log(error);
            }else{
                res.redirect('/ficha_tecnica');
